...
export default function PacoPlanWizard({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: "rgba(0,0,0,0.45)" }} onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="bg-(--ui-card) rounded-2xl shadow-2xl w-full max-w-md">
        <div className="flex items-center justify-between px-5 py-3.5" style={{ backgroundColor: COLOR }}>
          <p className="text-sm font-semibold text-(--g-on-accent)">Planificar con Paco 🦅</p>
        ...
        <div className="p-5 space-y-3">
          {mode === "menu" && (
            <>
              <p className="text-sm text-(--ui-text-3) mb-1">¿Qué quieres planificar?</p>
              {MENU_OPTIONS.map((opt) => (
                <button key={opt.label} onClick={opt.onClick} className="w-full text-left px-4 py-3 rounded-xl border border-(--ui-border) text-sm font-medium text-(--ui-text-2) hover:bg-(--ui-card-alt)">
                  {opt.label}
                </button>
              ))}
              <p className="text-xs text-(--ui-text-3) pt-1 leading-relaxed">
                La semana completa se arma desde <span className="font-medium text-(--ui-text-3)">Armar programación</span>,
          ...
          {mode === "dia" && (
            <>
              <p className="text-sm text-(--ui-text-3) mb-1">¿Para qué grupo y qué día?</p>
              <div className="flex flex-wrap gap-2">
                {GRUPOS.map((g) => (
                  <button key={g} onClick={() => { setGrupo(g); setDiaError(null); }} className="text-xs font-semibold px-3 py-1.5 rounded-full border"
                    style={grupo === g ? { backgroundColor: COLOR, color: "var(--g-on-accent)", borderColor: COLOR } : { color: "var(--ui-text-3)", borderColor: "var(--ui-border)" }}>
                    {TIPO_PLAN_LABEL[g]}
                  </button>
                ))}
              </div>
              <input type="date" value={fecha} onChange={(e) => { setFecha(e.target.value); setDiaError(null); }} className="w-full text-sm px-3 py-2 rounded-lg border border-(--ui-border)" />
              {diaError && <p className="text-xs text-(--ui-bad)">{diaError}</p>}
              <div className="flex gap-2 pt-2">
                <button onClick={() => setMode("menu")} className="flex-1 py-2 rounded-xl text-sm font-medium border border-(--ui-border) text-(--ui-text-2) hover:bg-(--ui-card-alt)">Atrás</button>
                <button onClick={handleConfirmDia} className="flex-1 py-2 rounded-xl text-sm font-semibold text-(--g-on-accent)" style={{ backgroundColor: COLOR }}>Continuar</button>
              </div>
            </>
          )}
          ...
          {mode === "especial" && (
            <>
              <p className="text-sm text-(--ui-text-3) mb-1">¿Qué grupos participan?</p>
              <div className="flex flex-wrap gap-2">
                {GRUPOS.map((g) => (
                  <button key={g} onClick={() => toggleGrupoMulti(g)} className="text-xs font-semibold px-3 py-1.5 rounded-full border"
                    style={gruposMulti.includes(g) ? { backgroundColor: "var(--ui-warn)", color: "var(--g-on-accent)", borderColor: "var(--ui-warn)" } : { color: "var(--ui-text-3)", borderColor: "var(--ui-border)" }}>
                    {TIPO_PLAN_LABEL[g]}
                  </button>
                ))}
                <button onClick={() => setGruposMulti(GRUPOS)} className="text-xs font-semibold px-3 py-1.5 rounded-full border"
                  style={gruposMulti.length === GRUPOS.length ? { backgroundColor: "var(--ui-warn)", color: "var(--g-on-accent)", borderColor: "var(--ui-warn)" } : { color: "var(--ui-text-3)", borderColor: "var(--ui-border)" }}>
                  Todos
                </button>
              </div>
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button
                  ...
                  className="flex-1 py-2 rounded-xl text-sm font-semibold text-(--g-on-accent)"
                  style={{ backgroundColor: "var(--ui-warn)" }}
                >
                  Continuar
                </button>
              </div>

...
export default function CalentamientoStep({ calentamiento, gruposFisico, usadosEnOtrasPartes, onChange }: Props) {
  ...
  if (!activo) {
    return (
      <div className="border border-dashed border-gray-200 rounded-xl p-4 flex items-center justify-between gap-3">
        <div>
          <p className="text-sm font-semibold text-gray-700">Calentamiento</p>
          <p className="text-xs text-gray-400 mt-0.5">Opcional — máx. {DURACION_MAX_MIN} min</p>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button onClick={() => onChange({ ejercicios: [], duracionMin: 8 })} className="text-xs font-medium text-blue-700 hover:text-blue-900">
            Elegir manualmente
          </button>
        </div>
      </div>
  ...
  return (
    <div className="border rounded-xl p-3 space-y-3" style={{ borderColor: calentamiento.ejercicios.length > 0 ? "#1B4D2E" : "#e5e7eb" }}>
      <div className="flex items-center justify-between">
        <p className="text-sm font-semibold text-gray-700">Calentamiento</p>
        <button onClick={() => onChange(null)} className="text-xs text-gray-400 hover:text-red-500">Saltar</button>
      </div>
